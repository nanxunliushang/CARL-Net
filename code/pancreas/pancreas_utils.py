...
def cutmix_config_log(save_path, tensorboard=False):
    ...
    logger = logging.getLogger(os.path.basename(os.path.dirname(save_path)))
...
def config_log(save_path, tensorboard=False):
    ...
    # 使用父目录名作为 logger 名称
    logger_name = save_path.parent.name  # 直接获取父目录名称
    ...
    # 控制台处理器
    sh = logging.StreamHandler(sys.stdout)
    sh.setFormatter(formatter)  # 修正：应为 sh 设置 formatter
    logger.addHandler(sh)
...
def LargestCC_pancreas(segmentation, connect_mode=1):
    ...
    return torch.as_tensor(np.array(batch_list)).cuda()
    #return torch.as_tensor(np.array(batch_list, dtype=np.float32)).cuda()
